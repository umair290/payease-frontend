...
const LocationPopup = ({ onAllow, onDeny }) => (
  <motion.div
    style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(10,20,50,0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px', boxSizing: 'border-box' }}
    initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
  >
    <motion.div
      style={{ background: '#fff', borderRadius: '24px', width: '100%', maxWidth: '340px', overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,0.4)' }}
    ...
    >
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg, #1A73E8, #0052CC)', padding: '24px 24px 20px', textAlign: 'center' }}>
        <motion.div
          style={{ width: '64px', height: '64px', borderRadius: '20px', background: 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}
        ...
        <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '12px', margin: 0 }}>
          PayEase would like to use your location
        </p>
      </div>
      ...
      <div style={{ padding: '20px 20px 24px' }}>
        <div style={{ marginBottom: '16px' }}>
          {[
            { icon: <Shield size={15} color="#1A73E8" />,    bg: 'rgba(26,115,232,0.08)',  text: 'Verify your login location for security' },
            { icon: <MapPin size={15} color="#16A34A" />,    bg: 'rgba(22,163,74,0.08)',   text: 'Detect suspicious logins from new places' },
            { icon: <CheckCircle size={15} color="#7C3AED" />, bg: 'rgba(124,58,237,0.08)', text: 'Your location is never shared with others' },
          ].map((item, i) => (
            <motion.div
              key={i}
              style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', background: item.bg, borderRadius: '10px', marginBottom: '6px' }}
              initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + i * 0.07 }}
            >
              {item.icon}
              <span style={{ color: '#1A1A2E', fontSize: '12px', fontWeight: '500' }}>{item.text}</span>
            </motion.div>
          ))}
        </div>

        <motion.button
          style={{ width: '100%', padding: '13px', background: 'linear-gradient(135deg, #1A73E8, #0052CC)', color: '#fff', border: 'none', borderRadius: '12px', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer', marginBottom: '8px', boxShadow: '0 4px 16px rgba(26,115,232,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
          whileTap={{ scale: 0.97 }} onClick={onAllow}
        >
          <MapPin size={16} color="#fff" /> Allow Location Access
        </motion.button>

        <motion.button
          style={{ width: '100%', padding: '12px', background: 'transparent', color: '#94A3B8', border: '1.5px solid #E2E8F0', borderRadius: '12px', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
          whileTap={{ scale: 0.97 }} onClick={onDeny}
        >
          <X size={14} color="#94A3B8" /> Not Now
        </motion.button>

        <p style={{ color: '#94A3B8', fontSize: '10px', textAlign: 'center', margin: '10px 0 0 0', lineHeight: '1.5' }}>
...
const ForgotModal = ({ show, onClose }) => {
  ...
  return (
    <AnimatePresence>
      {show && (
        <motion.div
          style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9998, padding: '20px', boxSizing: 'border-box' }}
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        >
          <motion.div
...
export default function Login() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setShowSplash(false);
      // Show location popup once per browser session after splash
      const locationAsked = sessionStorage.getItem('payease_location_asked');
      if (!locationAsked) {
  ...
  const handleAllowLocation = () => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (err) => {
          console.log('Location denied by browser:', err.message);
    ...
    try {
      ...
      setTimeout(() => {
        const locationStr = locationData
          ...
          : 'Location: not shared';
        logActivity('User Login', `Logged in from web browser — ${locationStr}`);
      }, 600);
          ...
          <motion.div key="login" style={styles.pageWrapper}
            ...
            <motion.div style={styles.card}
              ...
              <AnimatePresence>
                {locationGranted && (
                  <motion.div
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', background: 'rgba(22,163,74,0.08)', border: '1px solid rgba(22,163,74,0.2)', borderRadius: '20px', padding: '5px 12px', marginBottom: '14px', width: 'fit-content', margin: '0 auto 14px' }}
                    initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                  >
                    <MapPin size={12} color="#16A34A" />
                    <span style={{ color: '#16A34A', fontSize: '11px', fontWeight: '600' }}>Location verified for security</span>
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <motion.form onSubmit={handleLogin} style={styles.form}
                ...
                <Link to="/register" style={{ textDecoration: 'none' }}>
                  <motion.button
                    type="button" style={styles.registerBtn}
                    whileTap={{ scale: 0.98 }}
