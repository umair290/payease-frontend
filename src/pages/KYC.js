...
// ── Live Selfie Component — records VIDEO ────────────────────
function LiveSelfie({ onCapture, onCancel, isDark }) {
  ...
  const startChallenges = useCallback(() => {
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        const file = new File([blob], `liveness-video.${ext}`, { type: mimeType });
        setVideoBlob(file);
        setPhase('done');
      };
  ...
  const runChallenge = useCallback((idx) => {
    if (idx >= CHALLENGES.length) {
      // All done — capture thumbnail then stop recording
      ...
      <div style={{ position:'relative', width:'100%', flex:1, overflow:'hidden', maxHeight:'65vh' }}>
        ...
        {phase === 'done' && previewUrl && (
          <div style={{ width:'100%', height:'100%', background:'#000', display:'flex', alignItems:'center', justifyContent:'center', position:'relative' }}>
            ...
            <div style={{ position:'absolute', bottom:'16px', left:'50%', transform:'translateX(-50%)', background:'rgba(0,0,0,0.7)', borderRadius:'20px', padding:'6px 16px', backdropFilter:'blur(8px)' }}>
              <span style={{ color:'#4ADE80', fontSize:'12px', fontWeight:'700' }}>Video recorded successfully</span>
            </div>
          </div>
        )}
        ...
        {/* REC indicator */}
        {phase === 'challenge' && (
          <div style={{ position:'absolute', top:'70px', right:'16px', display:'flex', alignItems:'center', gap:'6px', background:'rgba(220,38,38,0.85)', borderRadius:'20px', padding:'5px 12px', backdropFilter:'blur(8px)' }}>
            <motion.div style={{ width:'8px', height:'8px', borderRadius:'50%', background:'#fff' }} animate={{ opacity:[1,0,1] }} transition={{ duration:1, repeat:Infinity }} />
            <span style={{ color:'#fff', fontSize:'11px', fontWeight:'700' }}>REC</span>
          </div>
        )}
        ...
        <div style={{ position:'absolute', top:0, left:0, right:0, padding:'20px 16px 12px', background:'linear-gradient(180deg,rgba(0,0,0,0.7) 0%,transparent 100%)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            ...
            <div style={{ background:'rgba(0,0,0,0.5)', borderRadius:'20px', padding:'6px 14px', backdropFilter:'blur(8px)' }}>
              <span style={{ color:'#fff', fontSize:'12px', fontWeight:'700' }}>Liveness Video Recording</span>
            </div>
      ...
      <div style={{ width:'100%', background: isDark ? '#0F1629' : '#fff', borderRadius:'24px 24px 0 0', padding:'20px 20px 40px', boxSizing:'border-box', flex:1 }}>
        ...
        {phase === 'intro' && (
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
            <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'12px' }}>
              <div style={{ width:'40px', height:'40px', borderRadius:'12px', background:'rgba(220,38,38,0.1)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                <motion.div style={{ width:'10px', height:'10px', borderRadius:'50%', background:'#DC2626' }} animate={{ opacity:[1,0,1] }} transition={{ duration:1, repeat:Infinity }} />
              </div>
              <div>
                <h3 style={{ color:text, fontSize:'16px', fontWeight:'800', margin:0 }}>Liveness Video Check</h3>
                <p style={{ color:textSec, fontSize:'11px', margin:0 }}>Session will be video recorded for admin review</p>
              </div>
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:'6px', marginBottom:'14px' }}>
              {CHALLENGES.map((c, i) => (
                <div key={c.id} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'9px 12px', background: isDark ? 'rgba(255,255,255,0.04)' : '#F8FAFF', borderRadius:'10px', border:`1px solid ${border}` }}>
                  <div style={{ width:'26px', height:'26px', borderRadius:'8px', background:c.grad, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                    <span style={{ color:'#fff', fontSize:'10px', fontWeight:'800' }}>{i+1}</span>
                  </div>
                  <span style={{ color:text, fontSize:'12px', fontWeight:'600', flex:1 }}>{c.label}</span>
                  <span style={{ color:textSec, fontSize:'10px' }}>{c.duration}s</span>
                </div>
              ))}
            </div>
            <div style={{ background:'rgba(220,38,38,0.06)', borderRadius:'12px', padding:'10px 14px', border:'1px solid rgba(220,38,38,0.15)', marginBottom:'14px', display:'flex', gap:'8px' }}>
              <Shield size={13} color="#DC2626" style={{ flexShrink:0, marginTop:'1px' }} />
              <p style={{ color:textSec, fontSize:'11px', margin:0, lineHeight:1.6 }}>
                The entire session is recorded. Admin watches your liveness video to verify your identity.
              </p>
            </div>
            <motion.button
              style={{ width:'100%', padding:'16px', background: cameraReady ? 'linear-gradient(135deg,#DC2626,#B91C1C)' : isDark ? 'rgba(255,255,255,0.06)' : '#F1F5F9', color: cameraReady ? '#fff' : textSec, border:'none', borderRadius:'16px', fontSize:'15px', fontWeight:'800', cursor: cameraReady ? 'pointer' : 'not-allowed', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px', boxShadow: cameraReady ? '0 8px 24px rgba(220,38,38,0.4)' : 'none' }}
              whileTap={cameraReady ? { scale:0.97 } : {}}
              onClick={() => cameraReady && startChallenges()}
            >
              {cameraReady
                ? <><Camera size={16} color="#fff" /> Start Recording</>
        ...
        {phase === 'challenge' && (
          <motion.div key={`ch-${challengeIdx}`} initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}>
            ...
            <div style={{ display:'flex', gap:'5px', flexWrap:'wrap' }}>
              {CHALLENGES.map((c, i) => (
                <div key={c.id} style={{ display:'flex', alignItems:'center', gap:'4px', padding:'4px 10px', background: completedIds.includes(c.id) ? 'rgba(22,163,74,0.1)' : i===challengeIdx ? 'rgba(220,38,38,0.1)' : isDark ? 'rgba(255,255,255,0.04)' : '#F1F5F9', borderRadius:'20px', border:`1px solid ${completedIds.includes(c.id) ? 'rgba(22,163,74,0.3)' : i===challengeIdx ? 'rgba(220,38,38,0.3)' : 'transparent'}` }}>
                  {completedIds.includes(c.id) && <CheckCircle size={10} color="#16A34A" />}
                  <span style={{ fontSize:'10px', fontWeight:'700', color: completedIds.includes(c.id) ? '#16A34A' : i===challengeIdx ? '#DC2626' : textSec }}>{c.id}</span>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        {phase === 'capturing' && (
          <motion.div style={{ textAlign:'center', padding:'16px 0' }} initial={{ opacity:0 }} animate={{ opacity:1 }}>
            ...
            <h3 style={{ color:text, fontSize:'17px', fontWeight:'800', margin:'0 0 6px 0' }}>Saving video...</h3>
            <p style={{ color:textSec, fontSize:'13px', margin:0 }}>Please hold still</p>
          </motion.div>
        )}
        ...
        {phase === 'done' && (
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
            <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'14px' }}>
              ...
              <div>
                <h3 style={{ color:text, fontSize:'17px', fontWeight:'800', margin:'0 0 3px 0' }}>Video Recorded!</h3>
                <p style={{ color:textSec, fontSize:'12px', margin:0 }}>All {CHALLENGES.length} liveness challenges completed</p>
              </div>
            </div>
            <div style={{ background: isDark ? 'rgba(22,163,74,0.06)' : 'rgba(22,163,74,0.04)', borderRadius:'12px', padding:'10px 14px', border:'1px solid rgba(22,163,74,0.2)', marginBottom:'14px', display:'flex', gap:'8px' }}>
              ...
              <p style={{ color:textSec, fontSize:'11px', margin:0, lineHeight:1.6 }}>
                Your liveness video will be reviewed by our team to verify your identity.
              </p>
            </div>
            <div style={{ display:'flex', gap:'10px' }}>
              ...
              <motion.button style={{ flex:2, padding:'14px', background:'linear-gradient(135deg,#16A34A,#15803D)', color:'#fff', border:'none', borderRadius:'14px', fontSize:'14px', fontWeight:'800', cursor:'pointer', boxShadow:'0 6px 20px rgba(22,163,74,0.4)', display:'flex', alignItems:'center', justifyContent:'center', gap:'6px' }} whileTap={{ scale:0.97 }} onClick={handleConfirm}>
                <CheckCircle size={15} color="#fff" /> Use This Video
              </motion.button>
            </div>
          </motion.div>
        )}
...
export default function KYC() {
  ...
  const [selfiePreview,  setSelfiePreview]  = useState(null);  // ← ADDED
  ...
  // ── UPDATED: receives video blob + thumbnail preview ──
  const handleSelfieCapture = async (blob, previewDataUrl) => {
    ...
    try {
      setSelfie(blob); // video — no compression needed
    } catch {
      setSelfie(blob);
    }
    setCompressing('');
            ...
            <motion.div key="s4" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }} transition={{ duration:0.25 }}>
                  ...
                  : selfie
                    ? <motion.div style={{ border:'2px dashed #16A34A', borderRadius:'18px', padding:'20px', textAlign:'center', background: isDark?'rgba(22,163,74,0.06)':'rgba(22,163,74,0.03)', marginBottom:'14px' }} initial={{ scale:0.95 }} animate={{ scale:1 }}>
                        <div style={{ width:'80px', height:'80px', borderRadius:'50%', overflow:'hidden', margin:'0 auto 12px', border:'3px solid #16A34A', boxShadow:'0 0 0 4px rgba(22,163,74,0.2)' }}>
                          {/* ── UPDATED: uses selfiePreview for thumbnail ── */}
                          <img src={selfiePreview || URL.createObjectURL(selfie)} alt="Selfie" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                        </div>
                        <p style={{ color:'#16A34A', fontSize:'14px', fontWeight:'800', margin:'0 0 3px 0' }}>Liveness Video Recorded!</p>
                        ...
                        <motion.div style={{ display:'inline-flex', alignItems:'center', gap:'6px', cursor:'pointer', padding:'6px 14px', background: isDark?'rgba(255,255,255,0.06)':'#F1F5F9', borderRadius:'10px', border:`1px solid ${border}` }} whileTap={{ scale:0.95 }} onClick={() => setShowLiveSelfie(true)}>
                          ...
                          <span style={{ color:textSec, fontSize:'11px', fontWeight:'700' }}>Retake video</span>
                        </motion.div>
                      </motion.div>
                    : <motion.div style={{ border:`2px dashed ${isDark?'rgba(255,255,255,0.1)':'#E2E8F0'}`, borderRadius:'18px', padding:'32px 20px', textAlign:'center', background: isDark?'rgba(255,255,255,0.02)':'#F8FAFF', marginBottom:'14px', cursor:'pointer' }} whileTap={{ scale:0.99 }} onClick={() => setShowLiveSelfie(true)}>
                        ...
                        <p style={{ color:textSec, fontSize:'12px', margin:'0 0 12px 0', lineHeight:1.5 }}>You'll be asked to perform 6 actions — the session is recorded as video</p>
                ...
                {!selfie && !compressing && (
                  <div style={{ background: isDark?'rgba(26,115,232,0.06)':'rgba(26,115,232,0.04)', borderRadius:'12px', padding:'12px 14px', border:`1px solid ${isDark?'rgba(26,115,232,0.15)':'rgba(26,115,232,0.1)'}`, display:'flex', gap:'10px' }}>
                    ...
                    <p style={{ color:textSec, fontSize:'12px', margin:0, lineHeight:'1.6' }}>
                      Look straight, turn left/right, look up, blink twice, smile. Full session recorded (~20 seconds).
                    </p>
                  </div>
                )}
              </motion.div>
              ...
              <motion.div style={{ background:card, borderRadius:'20px', overflow:'hidden', border:`1px solid ${border}`, marginBottom:'14px', boxShadow: isDark?'none':'0 4px 24px rgba(0,0,0,0.06)' }} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.05 }}>
                ...
                {[
                  ...
                  { label:'Live Video',   value:selfie?'Recorded':null, done:!!selfie },
                ].map((row, i, arr) => (
