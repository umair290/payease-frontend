...
export default function SendMoney() {
  ...
  const handlePrint = () => {
    ...
    <style>* { margin:0;padding:0;box-sizing:border-box; } body { font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#f0f4ff;display:flex;justify-content:center;padding:40px 20px; } .receipt { background:#fff;border-radius:20px;width:100%;max-width:400px;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.12); } .header { background:linear-gradient(135deg,#1A73E8,#0052CC);padding:32px;text-align:center; } .logo { color:#fff;font-size:24px;font-weight:bold;margin-bottom:16px; } .check { width:64px;height:64px;border-radius:50%;background:rgba(255,255,255,0.2);display:flex;align-items:center;justify-content:center;margin:0 auto 12px;font-size:28px; } .status { color:#fff;font-size:18px;font-weight:bold;margin-bottom:4px; } .type { color:rgba(255,255,255,0.7);font-size:13px; } .amount { color:#fff;font-size:36px;font-weight:bold;margin-top:12px; } .body { padding:24px; } .row { display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #f0f4ff; } .row:last-child { border-bottom:none; } .label { color:#888;font-size:13px; } .value { font-weight:600;font-size:13px;color:#1A1A2E;text-align:right;max-width:60%; } .footer { background:#f8faff;border-top:1px solid #e0e6f0;padding:16px;text-align:center; } .footer p { color:#888;font-size:11px;margin-bottom:4px; } @media print { body { background:white; } .receipt { box-shadow:none; } }</style></head>
    <body><div class="receipt"><div class="header"><div class="logo">PayEase</div><div class="check">&#10003;</div><div class="status">Transfer Successful</div><div class="type">Money Transfer</div><div class="amount">PKR ${parseFloat(amount).toLocaleString()}</div></div>
    <div class="body"><div class="row"><span class="label">To</span><span class="value">${recipient?.full_name}</span></div><div class="row"><span class="label">Phone</span><span class="value">${recipient?.phone}</span></div><div class="row"><span class="label">Wallet ID</span><span class="value">${walletNumber}</span></div><div class="row"><span class="label">Amount</span><span class="value" style="color:#1A73E8;font-size:15px">PKR ${parseFloat(amount).toLocaleString()}</span></div>${description ? `<div class="row"><span class="label">Note</span><span class="value">${description}</span></div>` : ''}<div class="row"><span class="label">Reference No.</span><span class="value">${txRef}</span></div><div class="row"><span class="label">Date and Time</span><span class="value">${txDate}</span></div><div class="row"><span class="label">Status</span><span class="value" style="color:#16A34A">Completed</span></div></div>
  ...
  if (success) return (
    <div style={{ minHeight: '100vh', background: colors.bg, maxWidth: '480px', margin: '0 auto' }}>
      ...
      <div style={{ padding: '16px' }}>
        <motion.div
          style={{ background: colors.card, borderRadius: '20px', overflow: 'hidden', border: `1px solid ${colors.border}`, marginBottom: '14px' }}
          ...
          <div style={{ padding: '4px 20px' }}>
            {[
              ...
              >
                ...
                <span style={{ color: row.color || colors.text, fontWeight: row.bold ? '700' : '600', fontSize: row.bold ? '16px' : '13px' }}>{row.value}</span>
              </motion.div>
            ))}
          </div>
        ...
        <motion.button
          style={{ width: '100%', padding: '15px', background: 'linear-gradient(135deg, #1A73E8, #0052CC)', color: '#fff', border: 'none', borderRadius: '14px', fontSize: '15px', fontWeight: 'bold', cursor: 'pointer', marginBottom: '10px', boxShadow: '0 6px 20px rgba(26,115,232,0.3)' }}
        ...
        <motion.button
          style={{ width: '100%', padding: '14px', background: 'transparent', color: colors.textSecondary, border: `1.5px solid ${colors.border}`, borderRadius: '14px', fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
  ...
  return (
    <div style={{ minHeight: '100vh', background: colors.bg, maxWidth: '480px', margin: '0 auto' }}>
      ...
      {/* Step Indicator */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '14px 32px', background: colors.card, borderBottom: `1px solid ${colors.border}` }}>
        {['Recipient', 'Amount', 'Confirm'].map((label, i) => (
          <React.Fragment key={i}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
              <motion.div
                style={{ width: '30px', height: '30px', borderRadius: '50%', background: step > i + 1 ? '#16A34A' : step === i + 1 ? '#1A73E8' : colors.actionBg, display: 'flex', alignItems: 'center', justifyContent: 'center', border: step >= i + 1 ? 'none' : `2px solid ${colors.border}` }}
              ...
              <span style={{ fontSize: '10px', color: step >= i + 1 ? colors.text : colors.textSecondary, fontWeight: step === i + 1 ? '600' : '400' }}>{label}</span>
            </div>
            {i < 2 && <div style={{ flex: 1, height: '2px', margin: '0 6px 14px', background: step > i + 1 ? '#16A34A' : colors.border, transition: 'background 0.3s' }} />}
          </React.Fragment>
        ))}
      </div>
          ...
          {step === 1 && (
            <motion.div key="step1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
              <div style={{ background: colors.card, borderRadius: '20px', padding: '24px', border: `1px solid ${colors.border}` }}>
                ...
                <AnimatePresence mode="wait">
                  {searchMode === 'wallet' && (
                    <motion.div key="wallet" style={{ marginBottom: '16px' }} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                      <p style={{ color: colors.textSecondary, fontSize: '12px', fontWeight: '600', margin: '0 0 8px 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Wallet Number</p>
                      <div style={{ display: 'flex', alignItems: 'center', border: `1.5px solid ${walletNumber ? '#1A73E8' : colors.border}`, borderRadius: '12px', padding: '0 16px', background: colors.inputBg, transition: 'all 0.2s', boxShadow: walletNumber ? '0 0 0 3px rgba(26,115,232,0.08)' : 'none' }}>
                        <CreditCard size={18} color={walletNumber ? '#1A73E8' : colors.textSecondary} style={{ flexShrink: 0, marginRight: '10px' }} />
                        <input
                          style={{ flex: 1, padding: '14px 0', border: 'none', background: 'transparent', color: colors.text, fontSize: '14px', outline: 'none' }}
                          placeholder="e.g. PK1234567890" value={walletNumber}
                  ...
                  {searchMode === 'phone' && (
                    <motion.div key="phone" style={{ marginBottom: '16px' }} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
                      <p style={{ color: colors.textSecondary, fontSize: '12px', fontWeight: '600', margin: '0 0 8px 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Phone Number</p>
                      <div style={{ display: 'flex', alignItems: 'center', border: `1.5px solid ${phoneNumber ? '#1A73E8' : colors.border}`, borderRadius: '12px', padding: '0 16px', background: colors.inputBg, transition: 'all 0.2s', boxShadow: phoneNumber ? '0 0 0 3px rgba(26,115,232,0.08)' : 'none' }}>
                        <Phone size={18} color={phoneNumber ? '#1A73E8' : colors.textSecondary} style={{ flexShrink: 0, marginRight: '10px' }} />
                        <input
                          style={{ flex: 1, padding: '14px 0', border: 'none', background: 'transparent', color: colors.text, fontSize: '14px', outline: 'none' }}
                          placeholder="03XXXXXXXXX" value={phoneNumber}
                ...
                {error && (
                  <motion.div style={{ background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: '10px', padding: '10px 14px', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                ...
                <motion.button
                  style={{ width: '100%', padding: '15px', background: (searchMode === 'wallet' ? walletNumber : phoneNumber) ? 'linear-gradient(135deg, #1A73E8, #0052CC)' : colors.actionBg, color: (searchMode === 'wallet' ? walletNumber : phoneNumber) ? '#fff' : colors.textSecondary, border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: 'bold', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', boxShadow: (searchMode === 'wallet' ? walletNumber : phoneNumber) ? '0 6px 20px rgba(26,115,232,0.3)' : 'none', transition: 'all 0.2s' }}
              ...
              <div style={{ background: colors.card, borderRadius: '16px', padding: '16px', marginBottom: '14px', border: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', gap: '14px' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  {recipient.kyc_verified
                    ? <><CheckCircle size={18} color="#16A34A" /><p style={{ color: '#16A34A', fontSize: '10px', fontWeight: '600', margin: '3px 0 0 0' }}>Verified</p></>
                    : <><Shield size={18} color="#CA8A04" /><p style={{ color: '#CA8A04', fontSize: '10px', fontWeight: '600', margin: '3px 0 0 0' }}>Pending</p></>
                  }
                </div>
              </div>
              ...
              <div style={{ background: colors.card, borderRadius: '20px', padding: '24px', border: `1px solid ${colors.border}` }}>
                ...
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', border: `2px solid ${amount ? '#1A73E8' : colors.border}`, borderRadius: '12px', padding: '0 16px', background: colors.inputBg, transition: 'all 0.2s', boxShadow: amount ? '0 0 0 3px rgba(26,115,232,0.08)' : 'none' }}>
                    ...
                    <input
                      style={{ flex: 1, padding: '16px 0', border: 'none', background: 'transparent', color: colors.text, fontSize: '24px', fontWeight: 'bold', outline: 'none' }}
                      type="number" placeholder="0" value={amount}
                      onChange={(e) => setAmount(e.target.value)}
                    />
                  </div>
                  <p style={{ color: colors.textSecondary, fontSize: '11px', margin: '6px 0 0 2px' }}>Maximum transfer: PKR 50,000 per transaction</p>
                </div>
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                  {[500, 1000, 5000, 10000].map(amt => (
                    <motion.button
                      key={amt}
                      style={{ flex: 1, padding: '8px 4px', background: amount == amt ? '#1A73E8' : colors.actionBg, color: amount == amt ? '#fff' : colors.textSecondary, border: `1px solid ${amount == amt ? '#1A73E8' : colors.border}`, borderRadius: '10px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', transition: 'all 0.2s' }}
                      whileTap={{ scale: 0.95 }} onClick={() => setAmount(String(amt))}
                    >
                ...
                <div style={{ marginBottom: '20px' }}>
                  <p style={{ color: colors.textSecondary, fontSize: '12px', fontWeight: '600', margin: '0 0 8px 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Note <span style={{ fontWeight: '400', textTransform: 'none' }}>(optional)</span></p>
                  <div style={{ display: 'flex', alignItems: 'center', border: `1.5px solid ${colors.border}`, borderRadius: '12px', padding: '0 16px', background: colors.inputBg }}>
                    ...
                    <input
                      ...
                      placeholder="e.g. Rent, Groceries..." value={description}
                ...
                {error && (
                  <motion.div style={{ background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: '10px', padding: '10px 14px', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                    <AlertCircle size={14} color="#DC2626" />
                    <span style={{ color: '#DC2626', fontSize: '13px' }}>{error}</span>
                  </motion.div>
                )}
                ...
                <motion.button
                  style={{ width: '100%', padding: '15px', background: amount ? 'linear-gradient(135deg, #1A73E8, #0052CC)' : colors.actionBg, color: amount ? '#fff' : colors.textSecondary, border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: 'bold', cursor: amount ? 'pointer' : 'not-allowed', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', boxShadow: amount ? '0 6px 20px rgba(26,115,232,0.3)' : 'none', transition: 'all 0.2s' }}
            ...
            <motion.div key="step3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1, background: 'rgba(255,255,255,0.12)', borderRadius: '14px', padding: '12px', textAlign: 'center' }}>
                    ...
                    <p style={{ color: '#fff', fontSize: '13px', fontWeight: '600', margin: '0 0 2px 0' }}>{recipient?.full_name?.split(' ')[0]}</p>
                    <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '10px', margin: 0 }}>Recipient</p>
                  </div>
                </div>
              </motion.div>
              ...
              >
                ...
                {[
                  ...
                  <motion.div
                    key={i}
                    style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '13px 16px', borderBottom: i < arr.length - 1 ? `1px solid ${colors.border}` : 'none' }}
                    initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 + i * 0.05 }}
                  >
                    <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: colors.actionBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {row.icon}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ color: colors.textSecondary, fontSize: '10px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.4px', margin: '0 0 2px 0' }}>{row.label}</p>
                      <p style={{ color: row.highlight ? '#1A73E8' : colors.text, fontSize: row.highlight ? '16px' : '13px', fontWeight: row.highlight ? '700' : '600', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.value}</p>
                    </div>
                  </motion.div>
                ))}
              </motion.div>
              ...
              <motion.div
                ...
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
              >
                <Shield size={16} color="#1A73E8" style={{ flexShrink: 0 }} />
                <p style={{ color: colors.textSecondary, fontSize: '12px', margin: 0, lineHeight: '1.5' }}>
                  Your PIN will be required to authorize this transfer. This transaction cannot be reversed once confirmed.
                </p>
              </motion.div>
              ...
              >
                <Lock size={16} color="#fff" /> Confirm and Enter PIN
              </motion.button>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <AnimatePresence>
        {showPinModal && (
          ...
          >
            ...
            <motion.div
              style={{ background: colors.card, borderRadius: '24px 24px 0 0', padding: '24px 24px 44px', width: '100%', maxWidth: '480px', boxSizing: 'border-box', position: 'relative', zIndex: 1 }}
            ...
            >
              ...
              <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                ...
                <h3 style={{ color: colors.text, fontSize: '18px', fontWeight: 'bold', margin: '0 0 4px 0' }}>Enter PIN</h3>
              ...
              <div style={{ background: colors.actionBg, borderRadius: '14px', padding: '14px 16px', marginBottom: '20px', border: `1px solid ${colors.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    <p style={{ color: colors.text, fontSize: '13px', fontWeight: '600', margin: 0 }}>{recipient?.full_name}</p>
                    <p style={{ color: colors.textSecondary, fontSize: '11px', margin: 0 }}>{recipient?.phone}</p>
                  </div>
                </div>
                <p style={{ color: '#1A73E8', fontSize: '18px', fontWeight: 'bold', margin: 0 }}>
                  PKR {parseFloat(amount).toLocaleString()}
                </p>
              </div>
              ...
              <div style={{ position: 'relative', marginBottom: '6px' }}>
                <input
                  ...
                  autoFocus
                />
                <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', padding: '16px', border: `2px solid ${pin.length === 4 ? '#1A73E8' : colors.border}`, borderRadius: '16px', background: colors.inputBg, transition: 'all 0.2s', boxShadow: pin.length === 4 ? '0 0 0 3px rgba(26,115,232,0.08)' : 'none' }}>
                  {[0, 1, 2, 3].map(i => (
                    <motion.div
                      key={i}
              ...
              {error && (
                <motion.div style={{ background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: '10px', padding: '10px 14px', margin: '8px 0 12px', display: 'flex', alignItems: 'center', gap: '8px' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
