...
const txRef = 'BILL' + Date.now().toString().slice(-8);
const txDate = new Date().toLocaleString('en-PK', {
...
export default function Bills() {
  ...
  const categories = {
    electricity: {
      icon: <Zap size={22} color="#FFB300" />,
      bg: 'rgba(255,179,0,0.1)', color: '#FFB300',
      label: 'Electricity', desc: 'Power & energy bills'
    },
    gas: {
      icon: <Wind size={22} color="#FF6B35" />,
      bg: 'rgba(255,107,53,0.1)', color: '#FF6B35',
      label: 'Gas', desc: 'Natural gas bills'
    },
    internet: {
      icon: <Wifi size={22} color="#1A73E8" />,
      bg: 'rgba(26,115,232,0.1)', color: '#1A73E8',
      label: 'Internet', desc: 'Broadband & DSL'
    },
    topup: {
      icon: <Phone size={22} color="#00C853" />,
      bg: 'rgba(0,200,83,0.1)', color: '#00C853',
      label: 'Mobile Top-up', desc: 'Prepaid recharge'
    },
  };
  ...
  const handlePrint = () => {
    ...
    const html = `
      <html><head><title>PayEase Bill Receipt</title>
      <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .logo { color:#fff; font-size:22px; font-weight:bold; margin-bottom:16px; }
        .check { width:60px; height:60px; border-radius:50%; background:rgba(255,255,255,0.2); display:flex; align-items:center; justify-content:center; margin:0 auto 12px; font-size:28px; }
        .status { color:#fff; font-size:18px; font-weight:bold; margin-bottom:4px; }
        .type { color:rgba(255,255,255,0.7); font-size:13px; }
        .amount { color:#fff; font-size:34px; font-weight:bold; margin-top:12px; }
        .body { padding:24px; }
        .row { display:flex; justify-content:space-between; padding:12px 0; border-bottom:1px solid #f0f4ff; }
        .row:last-child { border-bottom:none; }
        .label { color:#888; font-size:13px; }
        .value { font-weight:600; font-size:13px; color:#1A1A2E; text-align:right; }
      ...
      <body><div class="receipt">
        <div class="header">
          ...
          <div class="type">${cat?.label || selectedCategory} Bill</div>
          <div class="amount">PKR ${parseFloat(form.amount).toLocaleString()}</div>
        </div>
        <div class="body">
          <div class="row"><span class="label">Provider</span><span class="value">${selectedProvider}</span></div>
          <div class="row"><span class="label">Category</span><span class="value">${cat?.label || selectedCategory}</span></div>
          <div class="row"><span class="label">Reference No.</span><span class="value">${form.reference}</span></div>
          <div class="row"><span class="label">Amount Paid</span><span class="value" style="color:#1A73E8;font-size:15px">PKR ${parseFloat(form.amount).toLocaleString()}</span></div>
          <div class="row"><span class="label">Transaction ID</span><span class="value">${txRef}</span></div>
  ...
  const handleShare = () => {
    ...
    if (navigator.share) {
      navigator.share({ title: 'PayEase Bill Receipt', text });
    } else {
      ...
      <div style={{ background: 'linear-gradient(135deg, #1A73E8, #0052CC)', padding: '40px 24px 32px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        >
          <span style={{ color: '#fff', fontSize: '28px', fontWeight: 'bold' }}>
            PKR {parseFloat(form.amount).toLocaleString()}
          </span>
        </motion.div>
      </div>
        ...
        >
          ...
          <div style={{ padding: '16px 20px', background: colors.actionBg, borderBottom: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ background: 'rgba(0,200,83,0.1)', border: '1px solid rgba(0,200,83,0.3)', borderRadius: '20px', padding: '4px 12px', display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              <span style={{ color: '#00C853', fontSize: '11px', fontWeight: '700' }}>Paid</span>
            </div>
          </div>
          ...
          <div style={{ padding: '4px 20px' }}>
            {[
              ...
              { label: 'Status', value: '✓ Completed', color: '#00C853' },
            ].map((row, i, arr) => (
              <motion.div
                key={i}
              ...
              >
                ...
                <span style={{ color: row.color || colors.text, fontWeight: row.bold ? '700' : '600', fontSize: row.bold ? '15px' : '13px' }}>
                  {row.value}
                </span>
              </motion.div>
            ))}
        ...
        {['Category', 'Provider', 'Payment'].map((label, i) => (
          <React.Fragment key={i}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{ width: '22px', height: '22px', borderRadius: '50%', background: step > i + 1 ? '#00C853' : step === i + 1 ? '#1A73E8' : colors.actionBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {step > i + 1
                  ? <CheckCircle size={12} color="#fff" />
                  : <span style={{ color: step >= i + 1 ? '#fff' : colors.textSecondary, fontSize: '10px', fontWeight: '700' }}>{i + 1}</span>
                }
              </div>
              <span style={{ fontSize: '11px', fontWeight: step === i + 1 ? '600' : '400', color: step >= i + 1 ? colors.text : colors.textSecondary }}>
                {label}
              </span>
            </div>
            {i < 2 && <div style={{ flex: 1, height: '1.5px', background: step > i + 1 ? '#00C853' : colors.border, alignSelf: 'center', transition: 'background 0.3s' }} />}
          </React.Fragment>
        ))}
      </div>
            ...
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              {Object.keys(providers).map((category, i) => {
                ...
                return (
                  <motion.div
                    key={category}
                    style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '16px', background: colors.card, borderRadius: '16px', marginBottom: '10px', cursor: 'pointer', border: `1px solid ${colors.border}`, boxShadow: '0 2px 8px rgba(0,0,0,0.04)', transition: 'all 0.2s' }}
                    whileTap={{ scale: 0.98 }}
                    initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                    onClick={() => { setSelectedCategory(category); setStep(2); }}
                  >
                    <div style={{ width: '48px', height: '48px', borderRadius: '14px', background: c.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      {c.icon}
                    </div>
                    <div style={{ flex: 1 }}>
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              {(providers[selectedCategory] || []).map((provider, i) => (
                <motion.div
                  key={provider}
                  style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '16px', background: colors.card, borderRadius: '16px', marginBottom: '10px', cursor: 'pointer', border: `1px solid ${colors.border}`, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}
                  whileTap={{ scale: 0.98 }}
                  initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
            ...
            <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div style={{ background: colors.card, borderRadius: '16px', padding: '20px', border: `1px solid ${colors.border}`, marginBottom: '14px' }}>
                ...
                <div style={{ marginBottom: '14px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', border: `1.5px solid ${focusedField === 'ref' ? '#1A73E8' : form.reference ? '#1A73E8' : colors.border}`, borderRadius: '12px', padding: '0 14px', background: colors.inputBg, transition: 'all 0.2s', boxShadow: focusedField === 'ref' ? '0 0 0 3px rgba(26,115,232,0.08)' : 'none' }}>
                ...
                <div style={{ marginBottom: '4px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', border: `2px solid ${focusedField === 'amt' ? '#1A73E8' : form.amount ? '#1A73E8' : colors.border}`, borderRadius: '12px', padding: '0 14px', background: colors.inputBg, transition: 'all 0.2s', boxShadow: focusedField === 'amt' ? '0 0 0 3px rgba(26,115,232,0.08)' : 'none' }}>
              ...
              <div style={{ background: colors.card, borderRadius: '16px', padding: '20px', border: `1px solid ${colors.border}`, marginBottom: '14px' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    autoComplete="off"
                  />
                  <div style={{ display: 'flex', justifyContent: 'center', gap: '14px', padding: '16px', border: `2px solid ${pin.length === 4 ? '#1A73E8' : '#CBD5E0'}`, borderRadius: '14px', background: '#EEF2FF', transition: 'all 0.2s', boxShadow: pin.length === 4 ? '0 0 0 3px rgba(26,115,232,0.1)' : 'none' }}>
                    {[0, 1, 2, 3].map(i => (
                      <motion.div
                        key={i}
                        style={{ width: '52px', height: '52px', borderRadius: '14px', border: `2px solid ${i < pin.length ? '#1A73E8' : '#94A3B8'}`, background: i < pin.length ? 'rgba(26,115,232,0.12)' : '#E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: i < pin.length ? '0 4px 12px rgba(26,115,232,0.2)' : 'inset 0 2px 4px rgba(0,0,0,0.08)' }}
              ...
              {error && (
                <motion.div
                  style={{ background: 'rgba(255,68,68,0.08)', border: '1px solid rgba(255,68,68,0.2)', borderRadius: '12px', padding: '12px 16px', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}
              ...
              <motion.button
                style={{ width: '100%', padding: '15px', background: form.reference && form.amount && pin.length === 4 ? 'linear-gradient(135deg, #1A73E8, #0052CC)' : '#E2E8F0', color: form.reference && form.amount && pin.length === 4 ? '#fff' : '#94A3B8', border: 'none', borderRadius: '14px', fontSize: '15px', fontWeight: 'bold', cursor: 'pointer', boxShadow: form.reference && form.amount && pin.length === 4 ? '0 6px 20px rgba(26,115,232,0.3)' : 'none', transition: 'all 0.25s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
              ...
              >
                {loading
                  ...
                  : <><Lock size={16} color={form.reference && form.amount && pin.length === 4 ? '#fff' : '#94A3B8'} /> Pay PKR {form.amount ? parseFloat(form.amount).toLocaleString() : '0'}</>
                }
              </motion.button>
            </motion.div>
          )}
