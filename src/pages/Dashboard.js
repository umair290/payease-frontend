...
import {
  ...
  Sun, Moon, Bell, FileText, History
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const quickActions = [
    {
      icon: <ArrowUpRight size={22} color="#1A73E8" />,
      label: 'Send',
      bg: 'rgba(26,115,232,0.1)',
      action: () => navigate('/send')
    },
    {
      icon: <ArrowDownLeft size={22} color="#00C853" />,
      label: 'Deposit',
      bg: 'rgba(0,200,83,0.1)',
      action: () => setShowDeposit(true)
    },
    {
      icon: <FileText size={22} color="#FF6B35" />,
      label: 'Pay Bills',
      bg: 'rgba(255,107,53,0.1)',
      action: () => navigate('/bills')
    },
    {
      icon: <Clock size={22} color="#9C27B0" />,
      label: 'History',
    ...
    <div style={{ minHeight: '100vh', background: colors.bg, maxWidth: '480px', margin: '0 auto', position: 'relative' }}>
      ...
      >
        {quickActions.map((item, i) => (
          ...
          >
            ...
            <p style={{ color: colors.textSecondary, fontSize: '12px', margin: 0, fontWeight: '500' }}>
              {item.label}
            </p>
          </motion.div>
        ))}
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          >
            <span style={{ color: '#1A73E8', fontSize: '13px', fontWeight: '500' }}>All Transactions</span>
          ...
          transactions.slice(0, 6).map((tx, i) => (
            <motion.div
              ...
              style={{
                ...
                padding: '12px 0',
                borderBottom: i < transactions.slice(0, 6).length - 1 ? `1px solid ${colors.border}` : 'none',
                cursor: 'pointer'
              }}
            ...
            >
              <div style={{
                ...
                background: tx.direction === 'credit' ? 'rgba(0,200,83,0.1)' : 'rgba(26,115,232,0.1)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0
              }}>
                {tx.direction === 'credit'
                  ? <ArrowDownLeft size={20} color="#00C853" />
                  : <ArrowUpRight size={20} color="#1A73E8" />
                }
              </div>
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                ...
                <span style={{
                  ...
                  background: tx.status === 'completed' ? 'rgba(0,200,83,0.1)' : 'rgba(255,179,0,0.1)',
                  color: tx.status === 'completed' ? '#00C853' : '#FFB300',
                }}>
                  {tx.status}
                </span>
              </div>
        ...
        {navTabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <Icon
                size={22}
                color={isActive ? '#1A73E8' : colors.textSecondary}
      ...
      <AnimatePresence>
        {showDeposit && (
          <motion.div
            style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 1000 }}
          ...
          >
            ...
            >
              ...
              <div style={{ marginBottom: '12px' }}>
                ...
                <input
                  style={{ width: '100%', padding: '13px 16px', border: `1.5px solid ${colors.border}`, borderRadius: '12px', background: colors.inputBg, color: colors.text, fontSize: '14px', outline: 'none', boxSizing: 'border-box' }}
                  type="number"
