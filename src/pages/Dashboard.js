...
export default function Dashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      // ── Handle both paginated {transactions:[]} and legacy [] formats ──
  ...
  // ── Display name — use balance first, fallback to AuthContext user ──
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, maxWidth: '480px', margin: '0 auto', position: 'relative', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
          ...
          >
            ...
            >
              <div style={{ textAlign: 'center', marginBottom: '28px' }}>
                ...
                <h2 style={{ color: text, fontSize: '24px', fontWeight: '800', margin: '0 0 8px 0', letterSpacing: '-0.5px' }}>Welcome to PayEase</h2>
                <p style={{ color: textSec, fontSize: '14px', margin: 0, lineHeight: 1.6 }}>Your digital wallet is ready. Send money, pay bills, and manage your finances.</p>
              </div>
              {[
                { icon: <Send size={20} color="#1A73E8" />,   iconBg: 'rgba(26,115,232,0.1)', title: 'Send & Receive Money', desc: 'Transfer instantly to any PayEase wallet' },
                { icon: <Zap size={20} color="#EA580C" />,    iconBg: 'rgba(234,88,12,0.1)',  title: 'Pay Bills',           desc: 'Electricity, gas, internet and more' },
                { icon: <Shield size={20} color="#16A34A" />, iconBg: 'rgba(22,163,74,0.1)',  title: 'Bank-Grade Security', desc: 'Your money is protected at every step' },
              ].map((item, i) => (
                <motion.div key={i} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 0', borderBottom: i < 2 ? `1px solid ${border}` : 'none' }} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 + i * 0.1 }}>
                  <div style={{ width: '44px', height: '44px', borderRadius: '14px', background: item.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{item.icon}</div>
                  <div>
              ...
              <motion.button
                style={{ width: '100%', marginTop: '24px', padding: '17px', background: 'linear-gradient(135deg,#1A73E8,#7C3AED)', color: '#fff', border: 'none', borderRadius: '16px', fontSize: '16px', fontWeight: '800', cursor: 'pointer', boxShadow: '0 8px 28px rgba(26,115,232,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}
                whileTap={{ scale: 0.97 }} onClick={handleCompleteOnboarding}
              >
      ...
      <motion.div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 20px 16px' }} initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        <div>
          <p style={{ color: textSec, fontSize: '12px', margin: '0 0 3px 0', fontWeight: '500', letterSpacing: '0.3px' }}>{greeting}</p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <motion.div style={{ width: '40px', height: '40px', borderRadius: '13px', background: 'linear-gradient(135deg,#1A73E8,#7C3AED)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 16px rgba(26,115,232,0.35)', overflow: 'hidden' }} whileTap={{ scale: 0.88 }} onClick={() => navigate('/profile')}>
            {avatarUrl
              ...
              : <span style={{ color: '#fff', fontSize: '15px', fontWeight: '800' }}>{displayName.charAt(0).toUpperCase()}</span>
            }
          </motion.div>
        </div>
      </motion.div>
      ...
      {/* ── BALANCE CARD ── */}
      <div style={{ padding: '0 16px 4px' }}>
        <motion.div style={{ borderRadius: '24px', overflow: 'hidden', position: 'relative', background: 'linear-gradient(135deg,#1A1FEF 0%,#1A73E8 45%,#0EA5E9 100%)', boxShadow: '0 20px 60px rgba(26,115,232,0.45), 0 4px 16px rgba(0,0,0,0.2)' }} initial={{ opacity: 0, y: 30, scale: 0.96 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ delay: 0.1, type: 'spring', stiffness: 200, damping: 20 }}>
      ...
      {/* ── KYC BANNER — strictly only for unverified users ── */}
      {balance !== null && balance?.kyc_verified === false && (
        <motion.div
          ...
          whileTap={{ scale: 0.98 }} onClick={() => navigate('/kyc')}
        >
            ...
            {transactions.slice(0, 5).map((tx, i) => {
              ...
              return (
                <motion.div key={tx.id || i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '13px 18px', borderTop: `1px solid ${border}`, cursor: 'pointer' }} whileHover={{ background: isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.01)' }} whileTap={{ scale: 0.99 }} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.05 }}
                  onClick={() => {
                    setReceiptData({ type: getTxLabel(tx), amount: tx.amount, to: tx.to_wallet, from: tx.from_wallet, toName: tx.description, date: dateField ? new Date(String(dateField).replace(' ', 'T')).toLocaleString('en-PK') : 'N/A', ref: 'TXN' + (tx.id || Date.now().toString().slice(-8)), status: tx.status === 'completed' ? 'Successful' : tx.status, direction: tx.direction });
                    setShowReceipt(true);
                  }}
                >
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <p style={{ fontSize: '14px', fontWeight: '700', margin: '0 0 3px 0', color: tx.direction === 'credit' ? '#16A34A' : text }}>
                      {tx.direction === 'credit' ? '+' : '-'} PKR {Number(tx.amount).toLocaleString()}
                    </p>
            ...
            return (
              ...
              >
                {isScan ? (
                  ...
                  <>
                    <AnimatePresence>
                      {isActive && <motion.div style={{ position: 'absolute', top: '4px', width: '34px', height: '34px', borderRadius: '12px', background: 'rgba(26,115,232,0.12)' }} initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.6 }} transition={{ type: 'spring', stiffness: 400, damping: 25 }} />}
                    </AnimatePresence>
            ...
            <motion.div style={{ background: cardSolid, borderRadius: '28px 28px 0 0', padding: '24px 20px 44px', width: '100%', maxWidth: '480px', boxSizing: 'border-box', position: 'relative', zIndex: 1, boxShadow: '0 -8px 40px rgba(0,0,0,0.3)' }} initial={{ y: 500 }} animate={{ y: 0 }} exit={{ y: 500 }} transition={{ type: 'spring', damping: 28, stiffness: 280 }}>
              ...
              <div style={{ marginBottom: '14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', border: `2px solid ${depositAmount ? '#1A73E8' : border}`, borderRadius: '18px', padding: '0 20px', background: isDark ? 'rgba(255,255,255,0.04)' : '#F8FAFF', transition: 'all 0.2s', boxShadow: depositAmount ? '0 0 0 4px rgba(26,115,232,0.1)' : 'none' }}>
                  ...
                  <input style={{ flex: 1, padding: '18px 0', border: 'none', background: 'transparent', color: text, fontSize: '30px', fontWeight: '800', outline: 'none', letterSpacing: '-1px' }} type="number" placeholder="0" value={depositAmount} onChange={(e) => setDepositAmount(e.target.value)} autoFocus />
                </div>
              </div>
              <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
                {[500, 1000, 5000, 10000].map(amt => (
                  <motion.button key={amt} style={{ flex: 1, padding: '10px 4px', background: depositAmount == amt ? 'linear-gradient(135deg,#1A73E8,#0052CC)' : isDark ? 'rgba(255,255,255,0.05)' : '#F1F5F9', color: depositAmount == amt ? '#fff' : textSec, border: `1px solid ${depositAmount == amt ? 'transparent' : border}`, borderRadius: '12px', fontSize: '12px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s' }} whileTap={{ scale: 0.93 }} onClick={() => setDepositAmount(String(amt))}>
                    {amt >= 1000 ? `${amt/1000}K` : amt}
                  </motion.button>
                ))}
              </div>
              <motion.button style={{ width: '100%', padding: '17px', background: depositAmount ? 'linear-gradient(135deg,#16A34A,#15803D)' : isDark ? 'rgba(255,255,255,0.06)' : '#F1F5F9', color: depositAmount ? '#fff' : textSec, border: 'none', borderRadius: '16px', fontSize: '15px', fontWeight: '800', cursor: depositAmount ? 'pointer' : 'not-allowed', marginBottom: '10px', boxShadow: depositAmount ? '0 8px 24px rgba(22,163,74,0.35)' : 'none', transition: 'all 0.25s', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }} whileTap={depositAmount ? { scale: 0.97 } : {}} onClick={handleDeposit} disabled={actionLoading}>
                {actionLoading ? <><RefreshCw size={16} color="#fff" /> Processing...</> : <><Plus size={16} color={depositAmount ? '#fff' : textSec} /> Deposit PKR {depositAmount ? parseFloat(depositAmount).toLocaleString() : '0'}</>}
              </motion.button>
              <motion.button style={{ width: '100%', padding: '14px', background: 'transparent', color: textSec, border: `1.5px solid ${border}`, borderRadius: '14px', fontSize: '14px', cursor: 'pointer', fontWeight: '600', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }} whileTap={{ scale: 0.97 }} onClick={() => setShowDeposit(false)}>
